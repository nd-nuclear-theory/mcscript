"""qsubm -- generic queue submission for task-oriented batch scripts
    ...
    - A local definitions file config module must be defined in mcscript/config/
      and registered in mcscript/config/__init__.py.
    ...
    + 07/30/20 (pjf): Improve packaging of mcscript:
...
def main():
    ...
    if args.pool is None:
        job_name = "{:s}-{:s}".format(run, args.phase)
    else:
        job_name = "{:s}-{:s}-{:s}".format(run, args.pool, args.phase)
